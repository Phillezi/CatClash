...
int main(int argv, char **args)
{
    ...
    pPlayer->rect.x = windowWidth / 2;
    pPlayer->rect.y = windowHeight / 2;
...
int checkCollision(Player player, Tile map[], char direction)
{
    ...
    {
    case 'W':
        if (map[(((player.rect.y - 1) / TILESIZE) * MAPSIZE) + (player.rect.x / TILESIZE)].type)
            return map[(((player.rect.y - 1) / TILESIZE) * MAPSIZE) + (player.rect.x / TILESIZE)].type;
        else if (map[(((player.rect.y - 1) / TILESIZE) * MAPSIZE) + ((player.rect.x + (TILESIZE - 1)) / TILESIZE)].type)
            return map[(((player.rect.y - 1) / TILESIZE) * MAPSIZE) + ((player.rect.x + (TILESIZE - 1)) / TILESIZE)].type;
        break;
    case 'A':
        if (map[(((player.rect.y) / TILESIZE) * MAPSIZE) + ((player.rect.x - 1) / TILESIZE)].type)
            return map[(((player.rect.y) / TILESIZE) * MAPSIZE) + ((player.rect.x - 1) / TILESIZE)].type;
        else if (map[(((player.rect.y + (TILESIZE - 1)) / TILESIZE) * MAPSIZE) + ((player.rect.x - 1) / TILESIZE)].type)
            return map[(((player.rect.y + (TILESIZE - 1)) / TILESIZE) * MAPSIZE) + ((player.rect.x - 1) / TILESIZE)].type;
        break;
    case 'S':
        if (map[(((player.rect.y + TILESIZE) / TILESIZE) * MAPSIZE) + (player.rect.x / TILESIZE)].type)
            return map[(((player.rect.y + TILESIZE) / TILESIZE) * MAPSIZE) + (player.rect.x / TILESIZE)].type;
        else if (map[(((player.rect.y + TILESIZE) / TILESIZE) * MAPSIZE) + ((player.rect.x + (TILESIZE - 1)) / TILESIZE)].type)
            return map[(((player.rect.y + TILESIZE) / TILESIZE) * MAPSIZE) + ((player.rect.x + (TILESIZE - 1)) / TILESIZE)].type;
        break;
    case 'D':
        if (map[(((player.rect.y) / TILESIZE) * MAPSIZE) + ((player.rect.x + TILESIZE) / TILESIZE)].type)
            return map[(((player.rect.y) / TILESIZE) * MAPSIZE) + ((player.rect.x + TILESIZE) / TILESIZE)].type;
        else if (map[(((player.rect.y + (TILESIZE - 1)) / TILESIZE) * MAPSIZE) + ((player.rect.x + TILESIZE) / TILESIZE)].type)
            return map[(((player.rect.y + (TILESIZE - 1)) / TILESIZE) * MAPSIZE) + ((player.rect.x + TILESIZE) / TILESIZE)].type;
        break;
    default:
        break;
    }
